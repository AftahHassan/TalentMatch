...
- **THEN** the TalentMatch agent processes the message with full conversation history
- **THEN** the agent response is saved with role `assistant`
- **THEN** both messages are displayed in the chat UI
...
The TalentMatchAgent MUST use provided tools to access candidate data and MUST NOT fabricate information.

#### Scenario: Agent answers using candidate analysis
- **WHEN** the user asks a question about the candidate analysis
- **THEN** the agent calls `getCandidateAnalysis` to retrieve the analysis data
